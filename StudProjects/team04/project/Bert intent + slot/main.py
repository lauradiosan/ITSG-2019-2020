...
def main(args):
    ...
    if args.do_pred:
        trainer.load_model()
        predText = read_prediction_text(args)
        trainer.predict(predText, tokenizer)


def init(argss):
    init_logger()
    global tokenizer, trainer
    tokenizer = load_tokenizer(argss)

    train_dataset = None
    dev_dataset = None
    test_dataset = None

    trainer = Trainer(argss, train_dataset, dev_dataset, test_dataset)
    trainer.load_model()


def predict(text):
    return trainer.predictOne(text, tokenizer)
