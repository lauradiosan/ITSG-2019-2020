...
We also compute the precision against the CK+ emotion dataset and present the results here.\todo{here, the classifier was trained on FER and test on CK+ or it was trained from scratch by using CK+ data?}
...
\todo{because you state about a real-time approach, some statistics about the average testing time (per image or per dataset) could be added}

\todo{you can try to train the model by using the square hinge loss}

\todo{you can try to augment the training data (by variuos operations)
